...
function validateEnvConfig (prefix) {
  ...
  if (_.get(process.env, `${_prefix}AUTH_CLIENT_CERT_ENABLED`) &&
      (!_.get(process.env, `${_prefix}TLS_KEY`) ||
       (!_.get(process.env, `${_prefix}TLS_CERTIFICATE`)))) {
    throw new Error(
        `${_prefix}TLS_KEY and ${_prefix}TLS_CERTIFICATE required for ${_prefix}AUTH_CLIENT_CERT_ENABLED`)
  }
